    ...
    `,l=t`
      <section class="wallets-container">
        ...
        <ul class="wallets-body">
          ${this.getSortedList().map(((e,l)=>t`
                <li
                  @click=${()=>this.pickWalletOption(e)}
                  class=" wallets-body__item ${e.isAvailable?"":"not-available"} ${l===this.allowedWallets.value.length-1?"mb-0":""}">
                  <img src=${e.icon} alt=${e.name} />
                  <span class="dialog-text-solid">${e.name}</span>
                  ${e.isAvailable?"":t`<small class="not-available">${this.notAvailableText}</small>`}
                </li>
              `))}
        </ul>
      </section>
    `;return t`
      <dialog
        ...
        <section class="dialog-modal-body">
          ...
          <div class="dialog-modal-body__wallets">${l}</div>
        </section>
      </dialog>
