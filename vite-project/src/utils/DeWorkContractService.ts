...
const deWork = new Client({
    ...
    signTransaction: (async (xdrBase64: string, options) => {
        ...
        const publicKeyBuffer = Buffer.from(Keypair.fromPublicKey(address).rawPublicKey());
        ...
        const nextSequenceNumber = (BigInt(latestSequenceNumber) + BigInt(1)).toString();
        envelope.v1().tx().seqNum(xdr.Int64.fromString(nextSequenceNumber));

        const modifiedXdr = envelope.toXDR("base64");
        const transaction = new Transaction(modifiedXdr,"Test SDF Network ; September 2015" )
